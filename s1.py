...
class LogicSquareApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Kwadrat Logiczny")
        self.root.geometry("1200x960")

        self.LP = 1  # Logic Square counter
        self.squares = []  # List to store instances of Square class
        self.buttons = []  # List to store references to buttons
        self.entries = []  # List to store references to entry widgets
        self.expands = []
        self.create_widgets()
    ...
    def create_widgets(self, row_index=2):
        ...
        button1 = ttk.Button(self.root, text="+",
                             command=lambda i=row_index + 10: self.add_logic_square(i, entry1, entry2, entry3, entry4, 1))
        ...
        button2 = ttk.Button(self.root, text="+",
                             command=lambda i=row_index + 10: self.add_logic_square(i, entry1, entry2, entry3, entry4, 2))
        ...
        button3 = ttk.Button(self.root, text="+",
                             command=lambda i=row_index + 10: self.add_logic_square(i, entry1, entry2, entry3, entry4, 3))
    ...
    def add_extract_button(self, row_index):
        ...
        extract_button.grid(row=row_index+1, column=3, pady=10)
    ...
    def extract_and_expand(self):
        ...
        # Dodaj nowe wywołanie funkcji do przekształcania span_tree w directed graph
        directed_graph = self.span_tree_to_directed_graph(span_tree)

        # Dodaj nowe wywołanie funkcji do rysowania grafu z opisem przejść
    ...
    def add_user_transitions(self, directed_graph):
        ...
        dialog_window = tk.Toplevel(self.root)  # Zakładam, że masz atrybut self.root jako główne okno aplikacji

        for i, source in enumerate(all_nodes):
            for j, target in enumerate(all_nodes):
                if source != target:  # Wykluczamy przejścia od węzła do samego siebie
                    # Tworzymy etykietę i pole tekstowe dla każdego przejścia
                    label_text = f"Enter transition from {source} to {target}:"
                    label = ttk.Label(dialog_window, text=label_text)

                    entry_var = tk.StringVar()
                    entry = ttk.Entry(dialog_window, textvariable=entry_var)

                    # Pakujemy etykietę i pole tekstowe w oknie dialogowym
                    label.grid(row=i, column=0, padx=5, pady=5, sticky="e")
                    entry.grid(row=i, column=1, padx=5, pady=5, sticky="w")

                    # Dodajemy pole tekstowe do słownika
                    transitions[(source, target)] = entry_var

        # Funkcja obsługująca przycisk OK
        def ok_button_callback():
            # Wypisujemy wprowadzone przejścia
            for (source, target), entry_var in transitions.items():
                ...
                # Dodajemy krawędzie i etykiety zgodnie z wprowadzonymi przejściami
                if transition:
            ...
            # Po dodaniu przejść, rysujemy graf
            self.draw_directed_graph(directed_graph)

        # Dodajemy przycisk OK
        ok_button = ttk.Button(dialog_window, text="OK", command=ok_button_callback)
        ok_button.grid(row=len(all_nodes), column=0, columnspan=2, pady=10)

    def draw_directed_graph(self, directed_graph):
        pos = nx.spring_layout(directed_graph)  # Ustawienie pozycji wierzchołków grafu
        edge_labels = nx.get_edge_attributes(directed_graph, 'label')
        nx.draw(directed_graph, pos, with_labels=True, node_size=700, node_color='skyblue', font_size=8,
    ...
    def draw_tree(self, span_tree):
        ...
        queue = deque([(span_tree, None, None, 0)])  # (node, parent_name, xcenter, level)
    ...
    def draw_graph_in_tkinter(self, graph, pos):
        # Create a new Tkinter window for drawing the graph
